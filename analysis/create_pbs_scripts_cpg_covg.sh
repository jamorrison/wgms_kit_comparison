...
mkdir -p pbs_cpg_covg
mkdir -p cpg_covg

# Clean out directory if files already exist
rm -f pbs_cpg_covg/*.pbs
rm -f pbs_cpg_covg/submit_pbs_scripts.sh

# Create a bash script to loop through pbs scripts and submit them to the queue
cat > pbs_cpg_covg/submit_pbs_scripts.sh <<EOF
count=0
...
for FILE in `ls ${ALIGNDIR}/*.sorted.markdup.bam`; do
    ...
    sed "s/QQQ/${samp}/g" pbs_template_cpg_covg > pbs_cpg_covg/${samp}.pbs
done
