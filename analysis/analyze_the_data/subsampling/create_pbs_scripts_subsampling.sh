...
mkdir -p pbs_subsampling

# Clean out directory if files already exist
rm -f pbs_subsampling/*.pbs
rm -f pbs_subsampling/submit_pbs_scripts.sh

# Create a bash script to loop through pbs scripts and submit them to the queue
cat > pbs_subsampling/submit_pbs_scripts.sh <<EOF
count=0
...
for FILE in `ls ${ALIGNDIR}/*.sorted.markdup.bam`; do
    ...
    sed "s/QQQ/${samp}/g" pbs_template_subsampling > pbs_subsampling/${samp}.pbs
done
